-- Raise per-user active device cap from 1 to 8 (register_device RPC reads subscriptions.max_devices).

alter table public.subscriptions
  alter column max_devices set default 8;

update public.subscriptions
set max_devices = 8
where max_devices < 8;

-- Align RPC fallback when max_devices is unexpectedly null (should not happen with NOT NULL column).
